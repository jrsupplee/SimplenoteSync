...
sub uploadFileToNote {
	...
	if (eval { decode_utf8($content, Encode::FB_CROAK); 1 }) {
		...
		logIt( "$filepath is not a UTF-8 file. Will try to convert" );
...
sub downloadNoteToFile {
	...
	if ($content eq "") {
		...
		logIt( "$key no longer exists on server", 1 );
			...
			if ($file{"$directory/$filename"}{modify} eq $last_mod_date) {
				...
				if (defined ($note{$key}{modify})) {
					...
					if ($note{$key}{modify} eq $last_mod_date) {
						...
						logIt( "\tremote file is changed" );

						if ($note{$key}{deleted} eq "true") {
							# Remote note was flagged for deletion
							logIt( "Deleting $filename as it was deleted on server", 1 );
							if ($allow_local_updates) {
								File::Path::rmtree("$directory/$filename");
							...
							if (($newFile ne $filename) && ($newFile ne "")) {
								logIt( "Deleting $filename as it was renamed to $newFile", );
				...
				logIt( "\tlocal file has changed" );

				if ($note{$key}{modify} eq $last_mod_date) {
					# but note on server is old
					logIt( "\tbut server copy is unchanged" );
					...
					logIt( "$filename was modified locally and on server - please check file for conflicts.", 1 );
